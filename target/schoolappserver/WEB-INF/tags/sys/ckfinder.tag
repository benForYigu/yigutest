...
<%@ attribute name="horizontal" type="java.lang.Boolean" required="false" description="是否水平显示"%>
...
<script type="text/javascript">
    ...
    function ${input}Preview(){
        ...
        for (var i=0; i<urls.length; i++){
            if (urls[i]!=""){//<c:if test="${(type eq 'thumb' || type eq 'images')&& !horizontal}">
                li = "<li><img src=\""+urls[i]+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if>

                <c:if test="${(type eq 'thumb' || type eq 'images')&& horizontal}">
                li = "<img src=\""+urls[i]+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if>
                ...
                li += "&nbsp;&nbsp;<c:if test="${!readonly}"><a href=\"javascript:\" onclick=\"${input}Del(this);\">×</a></c:if>"+
                    "<c:if test="${(type eq 'thumb' || type eq 'images')&& horizontal}"></li></c:if>";

                <c:if test="${type ne 'thumb' && type eq 'flash'}">
                li = "<li><video width: 200px;\n" +
                    "    height: 200px;  controls=\"controls\" src=\""+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\">"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"</video>";
                li += "&nbsp;&nbsp;<c:if test="${!readonly}"><a href=\"javascript:\" onclick=\"${input}Del(this);\">×</a></c:if>"+
                    "<c:if test="${(type eq 'thumb' || type eq 'flash')&& horizontal}"></li></c:if>";
                //</c:if>
...
<%--
//<c:if test="${(type eq 'thumb' || type eq 'images')&& horizontal}">
    li = "<li><img src=\"" + urls[i] + "\" url=\"" + urls[i] + "\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${(type eq 'thumb' || type eq 'images')&& horizontal}">
    li = "<li><a href=\"" + urls[i] + "\" url=\"" + urls[i] + "\" target=\"_blank\">" + decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/") + 1)) + "</a>";//</c:if>
--%>
