...
public class UploadUtils {
	...
	// 最大文件大小
	private long maxSize = 1000000*1000;
	// 定义允许上传的文件扩展名
	...
	public UploadUtils() {
		...
		extMap.put("flashs", "swf,flv,mp4,MP4");
	...
	public UploadUtils(String url) {
		// 其中images,flashs,medias,files,对应文件夹名称,对应dirName
		// key文件夹名称
		// value该文件夹内可以上传文件的后缀名
		this.savePath=url;
		extMap.put("images", "gif,jpg,jpeg,png,bmp");
		extMap.put("flashs", "AVI,WMV,RM,RMVB,MPEG1,MPEG2,MPEG4,MP4,3GP,ASF,SWF,VOB,DAT,MOV,M4V,FLV,F4V,MKV,MTS,TS");
		extMap.put("medias", "swf,flv,mp3,wav,wma,wmv,mid,avi,mpg,asf,rm,rmvb");
		extMap.put("files", "doc,docx,xls,xlsx,ppt,htm,html,txt,zip,rar,gz,bz2");
	}
	...
	public List uploadFile(HttpServletRequest request) {
		...
		if (fiList != null) {
			for (FileItem item : fiList) {
				...
				if(infos[1].equals("true")){
					list.add("/schoolappserver"+url);
				}else{
					list.add(infos[1]);
				}
			}
...
//	@SuppressWarnings("unchecked")
	...
	private String saveFile(FileItem item) {
		...
		if (item.getSize() > maxSize) { // 检查文件大小
			// TODO
			error = "上传文件大小超过限制";
		} else if (!dirName.equals("files")&&!Arrays.<String> asList(extMap.get(dirName).toLowerCase().split(",")).contains(fileExt)) {// 检查扩展名
