package com.thinkgem.jeesite.API.weixin.util;


import com.thinkgem.jeesite.API.weixin.bean.jssdk.JsConfig;
import com.thinkgem.jeesite.API.weixin.bean.pay.PayPackage;
...
public class JsUtil {
    ...
    public static JsConfig generateConfigJsonWrapper(String jsapi_ticket, boolean debug, String appId, String url, String... jsApiList) {
        long timestamp = System.currentTimeMillis() / 1000;
        String nonceStr = UUID.randomUUID().toString();
        String signature = generateConfigSignature(nonceStr, jsapi_ticket, timestamp + "", url);
        JsConfig config = new JsConfig(debug, appId, timestamp, nonceStr, signature, jsApiList == null ? ALL_JS_API_LIST : jsApiList);
        return config;
    }
    public static JsConfig generateConfigJsonWrapper(String jsapi_ticket, boolean debug, String appId, String url, Long accountId,String... jsApiList) {
        long timestamp = System.currentTimeMillis() / 1000;
        String nonceStr = UUID.randomUUID().toString();
        String signature = generateConfigSignature(nonceStr, jsapi_ticket, timestamp + "", url);
        JsConfig config = new JsConfig(debug, appId, timestamp, nonceStr, signature, jsApiList == null ? ALL_JS_API_LIST : jsApiList,accountId);
        return config;
    }
    /**
     * 生成微信支付JSON
