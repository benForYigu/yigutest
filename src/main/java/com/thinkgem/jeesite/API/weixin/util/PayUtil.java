...
import com.thinkgem.jeesite.API.weixin.bean.pay.PayAppRequest;
import com.thinkgem.jeesite.API.weixin.bean.pay.PayJsRequest;
...
/**
    ...
    public static String generateMchPayAppRequestJson(String prepayId,String merId,String appId, String key) {
        String package_ = "Sign=WXPay";
        PayAppRequest payAppRequest = new PayAppRequest();
        payAppRequest.setAppid(appId);

        payAppRequest.setPartnerid(merId);
        payAppRequest.setPrepayid(prepayId);

        payAppRequest.setNoncestr(UUID.randomUUID().toString().replace("-", ""));
        payAppRequest.setPackage_(package_);

        payAppRequest.setTimestamp(System.currentTimeMillis() / 1000 + "");
        //@fantycool 提交修正bug
        //Map<String, String> mapS = MapUtil.objectToMap(payJsRequest,"signType","paySign");
        Map<String, String> mapS = MapUtil.objectToMap(payAppRequest);
        String sign = SignatureUtil.generateSign(mapS, key);
        payAppRequest.setSign(sign);
        return JsonUtil.toJSONString(payAppRequest);
    }
